...
export class ExamTopicConfigController {
  ...
  @ApiOperation({ summary: 'Setup or update question count distribution for a topic in an exam (Admin / Lecturer only)' })
  @ApiResponse({ status: 201, description: 'Configuration upserted successfully' })
  @ApiResponse({ status: 400, description: 'Question count cannot be satisfied due to dependency chains / locks active' })
  @ApiResponse({ status: 404, description: 'Exam or Topic not found' })
  upsertConfig(@Body() dto: CreateExamTopicConfigDto, @CurrentUser() user: User) {
    return this.configService.upsertConfig(dto, user);
  }

  @Delete(':examId/:topicId')
  @Roles(Role.ADMIN, Role.LECTURER)
  @ApiOperation({ summary: 'Remove question count distribution for a topic in an exam (Admin / Lecturer only)' })
  @ApiResponse({ status: 200, description: 'Configuration deleted successfully' })
  @ApiResponse({ status: 400, description: 'Config cannot be deleted because locks are active' })
