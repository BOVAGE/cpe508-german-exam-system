...
export class CourseAssignmentController {
  ...
  @Post()
  @Roles(Role.ADMIN)
  @ApiOperation({ summary: 'Assign a Lecturer to a Course for a session/academic year (Admin only)' })
  @ApiResponse({ status: 201, description: 'Course assignment created successfully' })
  @ApiResponse({ status: 400, description: 'Lecturer already assigned or user role mismatch' })
  ...
  @ApiResponse({ status: 200, description: 'List of course assignments retrieved' })
  findAll() {
  ...
  @Get(':id')
  @Roles(Role.ADMIN)
  @ApiOperation({ summary: 'Get specific Course Assignment details (Admin only)' })
