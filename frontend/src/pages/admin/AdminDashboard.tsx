...
const AdminDashboard: React.FC = () => {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: [variables.path.replace(/^\//, '')] });
  ...
  const handleExcelImport = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(err.response?.data?.message || 'Failed to import student accounts.');
    } finally {
  ...
  const getInitialFormState = (entity: string) => {
    switch (entity) {
      case 'faculty': return { name: '', code: '' };
      case 'department': return { name: '', code: '', facultyId: '' };
      case 'course': return { name: '', code: '', departmentId: '' };
      case 'lecturer': return { email: '', password: '', firstName: '', lastName: '', departmentId: '' };
      case 'student': return { email: '', password: '', registrationNumber: '', firstName: '', lastName: '', departmentId: '' };
      case 'assignment': return { lecturerId: '', courseId: '', academicYear: '2025/2026', session: 'Rain' };
      default: return {};
    }
  };
  ...
  const mapItemToForm = (entity: string, item: any) => {
    switch (entity) {
      case 'faculty': return { name: item.name, code: item.code };
      case 'department': return { name: item.name, code: item.code, facultyId: item.facultyId };
      case 'course': return { name: item.name, code: item.code, departmentId: item.departmentId };
      case 'lecturer': return { email: item.email, firstName: item.firstName, lastName: item.lastName, departmentId: item.departmentId || '' };
      case 'student': return { email: item.email, registrationNumber: item.registrationNumber, firstName: item.firstName, lastName: item.lastName, departmentId: item.departmentId || '' };
      default: return {};
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Configure academic hierarchies, lecturers assignments, and students records.</p>
        </div>
      ...
      <div className="animate-in fade-in duration-200">
        ...
        {activeTab === 'overview' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { label: 'Faculties', count: faculties.length, icon: Building2, color: 'text-sky-400' },
              { label: 'Departments', count: departments.length, icon: Building2, color: 'text-indigo-400' },
              { label: 'Courses', count: courses.length, icon: BookOpen, color: 'text-violet-400' },
              { label: 'Lecturers', count: lecturers.length, icon: Users, color: 'text-amber-400' },
              { label: 'Students', count: students.length, icon: Users, color: 'text-emerald-400' },
              { label: 'Course Assignments', count: assignments.length, icon: UserPlus, color: 'text-rose-400' },
            ].map((card, i) => {
              ...
              return (
                <div key={i} className="glass-panel p-6 flex items-center justify-between">
                  ...
                  <div className={`p-4 bg-slate-950/80 border border-slate-800 rounded-2xl ${card.color}`}>
        ...
        {activeTab === 'faculties' && (
          <div className="glass-panel overflow-hidden">
            <div className="p-4 border-b border-slate-800 flex items-center justify-between">
              <h2 className="font-bold text-sm text-slate-300">Faculties Directory</h2>
              <button 
              ...
              <tbody className="divide-y divide-slate-800/60">
                {filterList(faculties, ['name', 'code']).map((fac: any) => (
                  <tr key={fac.id} className="hover:bg-slate-900/20">
                    <td className="p-4 font-semibold text-slate-200">{fac.name}</td>
                    <td className="p-4 font-mono text-xs">{fac.code}</td>
                    <td className="p-4 text-right space-x-2">
                      <button onClick={() => handleOpenEdit('faculty', fac)} className="p-1.5 hover:bg-slate-800 text-slate-400 hover:text-slate-100 rounded">
                        <Edit2 className="h-3.5 w-3.5" />
                      </button>
                      <button onClick={() => handleOpenDelete('faculty', fac)} className="p-1.5 hover:bg-slate-800 text-red-500/80 hover:text-red-400 rounded">
        ...
        {activeTab === 'departments' && (
          <div className="glass-panel overflow-hidden">
            <div className="p-4 border-b border-slate-800 flex items-center justify-between">
              <h2 className="font-bold text-sm text-slate-300">Departments Directory</h2>
              <button 
              ...
              <tbody className="divide-y divide-slate-800/60">
                {filterList(departments, ['name', 'code']).map((dept: any) => (
                  <tr key={dept.id} className="hover:bg-slate-900/20">
                    ...
                    <td className="p-4 text-right space-x-2">
                      <button onClick={() => handleOpenEdit('department', dept)} className="p-1.5 hover:bg-slate-800 text-slate-400 hover:text-slate-100 rounded">
                        <Edit2 className="h-3.5 w-3.5" />
                      </button>
                      <button onClick={() => handleOpenDelete('department', dept)} className="p-1.5 hover:bg-slate-800 text-red-500/80 hover:text-red-400 rounded">
                        <Trash2 className="h-3.5 w-3.5" />
                      </button>
                    </td>
                  </tr>
        ...
        {activeTab === 'courses' && (
          <div className="glass-panel overflow-hidden">
            <div className="p-4 border-b border-slate-800 flex items-center justify-between">
              <h2 className="font-bold text-sm text-slate-300">Courses Directory</h2>
              <button 
              ...
              <tbody className="divide-y divide-slate-800/60">
                {filterList(courses, ['name', 'code']).map((course: any) => (
                  <tr key={course.id} className="hover:bg-slate-900/20">
                    <td className="p-4 font-semibold text-slate-200">{course.name}</td>
                    <td className="p-4 font-mono text-xs text-indigo-400">{course.code}</td>
                    <td className="p-4 text-slate-400">{course.department?.name}</td>
                    <td className="p-4 text-right space-x-2">
                      <button onClick={() => handleOpenEdit('course', course)} className="p-1.5 hover:bg-slate-800 text-slate-400 hover:text-slate-100 rounded">
                        <Edit2 className="h-3.5 w-3.5" />
                      </button>
        ...
        {activeTab === 'lecturers' && (
          <div className="glass-panel overflow-hidden">
            <div className="p-4 border-b border-slate-800 flex items-center justify-between">
              <h2 className="font-bold text-sm text-slate-300">Lecturers Directory</h2>
              <button 
            ...
            <table className="w-full text-left text-sm text-slate-300">
              ...
              <tbody className="divide-y divide-slate-800/60">
                {filterList(lecturers, ['firstName', 'lastName', 'email']).map((lec: any) => (
                  <tr key={lec.id} className="hover:bg-slate-900/20">
                    <td className="p-4 font-semibold text-slate-200">{lec.firstName} {lec.lastName}</td>
                    <td className="p-4 font-mono text-xs">{lec.email}</td>
                    <td className="p-4 text-slate-400">
                      {departments.find((d: any) => d.id === lec.departmentId)?.name || 'N/A'}
                    </td>
                    <td className="p-4 text-right space-x-2">
                      <button onClick={() => handleOpenEdit('lecturer', lec)} className="p-1.5 hover:bg-slate-800 text-slate-400 hover:text-slate-100 rounded">
                        <Edit2 className="h-3.5 w-3.5" />
                      </button>
                      <button onClick={() => handleOpenDelete('lecturer', lec)} className="p-1.5 hover:bg-slate-800 text-red-500/80 hover:text-red-400 rounded">
                        <Trash2 className="h-3.5 w-3.5" />
                      </button>
                    </td>
                  </tr>
        ...
        {activeTab === 'students' && (
          <div className="glass-panel overflow-hidden">
            <div className="p-4 border-b border-slate-800 flex items-center justify-between">
              <h2 className="font-bold text-sm text-slate-300">Students Directory</h2>
              <button 
            ...
            <table className="w-full text-left text-sm text-slate-300">
              ...
              <tbody className="divide-y divide-slate-800/60">
                {filterList(students, ['firstName', 'lastName', 'registrationNumber', 'email']).map((stud: any) => (
                  <tr key={stud.id} className="hover:bg-slate-900/20">
                    <td className="p-4 font-semibold text-slate-200">{stud.firstName} {stud.lastName}</td>
                    <td className="p-4 font-mono text-xs text-indigo-400">{stud.registrationNumber}</td>
                    <td className="p-4 font-mono text-xs">{stud.email}</td>
                    <td className="p-4 text-slate-400">
                      {departments.find((d: any) => d.id === stud.departmentId)?.name || 'N/A'}
                    </td>
                    <td className="p-4 text-right space-x-2">
                      <button onClick={() => handleOpenEdit('student', stud)} className="p-1.5 hover:bg-slate-800 text-slate-400 hover:text-slate-100 rounded">
                        <Edit2 className="h-3.5 w-3.5" />
                      </button>
                      <button onClick={() => handleOpenDelete('student', stud)} className="p-1.5 hover:bg-slate-800 text-red-500/80 hover:text-red-400 rounded">
                        <Trash2 className="h-3.5 w-3.5" />
                      </button>
                    </td>
                  </tr>
        ...
        {activeTab === 'assignments' && (
          <div className="glass-panel overflow-hidden">
            <div className="p-4 border-b border-slate-800 flex items-center justify-between">
              <h2 className="font-bold text-sm text-slate-300">Lecturer Course Assignments</h2>
              <button 
            ...
            <table className="w-full text-left text-sm text-slate-300">
              ...
              <tbody className="divide-y divide-slate-800/60">
                {filterList(assignments, ['academicYear', 'session']).map((assign: any) => {
                  const lec = lecturers.find((l: any) => l.id === assign.lecturerId);
                  const course = courses.find((c: any) => c.id === assign.courseId);
                  return (
                    <tr key={assign.id} className="hover:bg-slate-900/20">
                      ...
                      <td className="p-4 text-slate-300">{assign.academicYear}</td>
                      <td className="p-4 font-mono text-xs">{assign.session}</td>
                      <td className="p-4 text-right">
                        <button onClick={() => handleOpenDelete('assignment', assign)} className="p-1.5 hover:bg-slate-800 text-red-500/80 hover:text-red-400 rounded">
                          <Trash2 className="h-3.5 w-3.5" />
                        </button>
                      </td>
                    </tr>
            ...
            {modalType === 'delete' ? (
              <div className="space-y-4">
                <p className="text-slate-300 text-sm">
                  Are you absolutely sure you want to delete this {currentEntity}? This action is permanent and may break associated records.
                </p>
                <div className="flex justify-end gap-3 pt-2">
                  <button onClick={closeFormModal} className="glass-btn-secondary py-2 px-4">Cancel</button>
                  <button onClick={handleDeleteConfirm} className="glass-btn-danger py-2 px-4">Confirm Delete</button>
                </div>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-4 text-left">
                {/* Faculty & Department Forms */}
                {(currentEntity === 'faculty' || currentEntity === 'department' || currentEntity === 'course') && (
                  <>
                ...
                {currentEntity === 'department' && (
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-slate-400">Faculty Selection</label>
                    <select
                      required
                ...
                {currentEntity === 'course' && (
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-slate-400">Department Selection</label>
                    <select
                      required
                ...
                {(currentEntity === 'lecturer' || currentEntity === 'student') && (
                  <>
                ...
                {currentEntity === 'assignment' && (
                  <>
                    <div className="space-y-1.5">
                      <label className="text-xs font-semibold text-slate-400">Lecturer</label>
                      <select
                        required
                ...
                <div className="flex justify-end gap-3 pt-2">
                  <button type="button" onClick={closeFormModal} className="glass-btn-secondary py-2 px-4">Cancel</button>
                  <button type="submit" className="glass-btn-primary py-2 px-4">Save Configuration</button>
                </div>
              </form>
            )}
          </div>
        ...
        <div className="glass-modal-overlay">
          <div className="glass-modal-content max-w-xl">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold">Bulk Import Student Accounts</h3>
            ...
            <form onSubmit={handleExcelImport} className="space-y-4">
              ...
              <div className="border-2 border-dashed border-slate-800 rounded-xl p-8 flex flex-col items-center justify-center hover:border-slate-700 transition-colors relative">
                ...
                {selectedFile ? (
                  ...
                  <p className="text-xs text-slate-500">Drag & drop or click to select Excel sheet</p>
                )}
              </div>

              {/* Action buttons */}
              <div className="flex justify-end gap-3">
                <button type="button" onClick={() => setImportModalOpen(false)} className="glass-btn-secondary">Cancel</button>
                <button type="submit" disabled={importLoading || !selectedFile} className="glass-btn-primary">
            ...
            {importResult && (
              <div className="mt-6 border-t border-slate-800 pt-4 space-y-4">
                <h4 className="text-sm font-bold text-slate-300">Import Summary</h4>
                ...
                <div className="grid grid-cols-2 gap-4 text-xs">
                  <div className="p-3 bg-emerald-500/10 border border-emerald-500/30 rounded-lg flex items-center space-x-2">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-bold text-emerald-400">{importResult.importedCount ?? 0}</p>
                    </div>
                  </div>
